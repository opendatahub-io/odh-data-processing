    ...
    if "notebook_path" in metafunc.fixturenames:
        ...
        if changed_notebooks_env:
            ...
            try:
                ...
                notebook_paths = [Path(file) for file in changed_files if file.endswith('.ipynb')]
                existing_notebooks = [path for path in notebook_paths if path.exists()]
                
                if existing_notebooks:
                    metafunc.parametrize("notebook_path", existing_notebooks, ids=lambda x: x.name)
                    return
        ...
        if notebooks_dir.exists():
            ...
            if notebook_files:
                metafunc.parametrize("notebook_path", notebook_files, ids=lambda x: x.name)
            else:
...
def test_notebook_clean_state(notebook_path):
    ...
    assert not output_errors, f"Notebook has cell outputs:\n" + "\n".join(output_errors)
    
    count_errors = validator.validate_no_execution_counts()
    assert not count_errors, f"Notebook has execution counts:\n" + "\n".join(count_errors)
...
def test_notebook_has_parameters(notebook_path):
    ...
    assert not param_errors, f"Notebook missing parameters cell:\n" + "\n".join(param_errors)