...
class EncoderConfig:
    ...
    encoder_model_path: str | None = field(default=None, metadata={"advanced": True})  
...
class SystemConfig:
    ...
    def __post_init__(self):
        ...
        self.num_gpus = get_default_num_gpus()  
...
class DataProcessor:
    ...
    def generate_embeddings(self, dataset, output_dir: str) -> str:
        ...
        if torch.cuda.is_available():
            ...
            # In testing mode without GPU, use 1 CPU "worker"
            num_gpus = 1
        logger.info(
            f"Using {num_gpus} {'GPU' if torch.cuda.is_available() else 'CPU worker'}{'s' if num_gpus > 1 else ''} for embedding generation"
        )
        ...
        for gpu_id in range(num_gpus):
            ...
            args_list.append(
                (
                    ...
                    self.config.encoder.encoder_model_path,  # Pass model_path
                )
            )
...
def _process_dataset_shard(args):
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            device = "cpu"
            logger.info(
                f"CPU worker {gpu_id} started processing {len(dataset_shard)} samples"
            )
        ...
        encoder = encoder_cls(
            ...
            model_path=encoder_model_path,  # Pass model_path
        )
        ...
        # Create progress bar
        device_name = (
            f"GPU {gpu_id}" if torch.cuda.is_available() else f"CPU worker {gpu_id}"
        )
        progress_bar = tqdm(
            desc=f"{device_name} generating embeddings",
        ...
        if not all_embeddings:
            device_label = "GPU" if torch.cuda.is_available() else "CPU worker"
            logger.warning(
                f"No embeddings generated for shard on {device_label} {gpu_id}"
            )
            return None
        ...
        device_label = "GPU" if torch.cuda.is_available() else "CPU worker"
        logger.info(
            f"{device_label} {gpu_id} completed processing. Saved to {shard_file}"
        )
    ...
    # pylint: disable=broad-exception-caught
        ...
        device_label = "GPU" if torch.cuda.is_available() else "CPU worker"
        logger.error(f"Error processing shard on {device_label} {gpu_id}: {str(e)}")
...
def _merge_shard_files(shard_files, merged_file):
    ...
    device_label = "GPUs" if torch.cuda.is_available() else "CPU workers"
    logger.info(
        f"Successfully merged embeddings from {len(shard_files)} {device_label} with {total_samples} total samples"
    )
...
def process_folds_with_gpu(args):
    ...
    try:
        if torch.cuda.is_available():
            ...
            logger.warning(
                "Running in CPU mode. Production use requires GPU acceleration."
            )
            device = "cpu"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Comma-separated subset sizes. Use percentages (e.g., '10%%,50%%') or absolute counts (e.g., '1000,5000'). "
        "Decimal notation (e.g., '0.1,0.5') is also supported for backward compatibility.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Local path to encoder model (optional, will download from HuggingFace if not provided)",
    )
...
def main():
    ...
    for size_str in args.subset_sizes.split(","):
        ...
        # Handle percentage notation (e.g., "10%", "50%")
        if size_str.endswith("%"):
            ...
            subset_sizes.append(percentage / 100.0)  

        # Handle decimal notation
        elif "." in size_str:
            value = float(size_str)
            if 0 < value <= 1:
                # Treat as decimal percentage (0.1 = 10%)
                subset_sizes.append(value)
            else:
                raise ValueError(
                    f"Decimal values must be between 0 and 1, got: {value}"
                )

        # Handle absolute count
        else:
            count = int(size_str)
            if count <= 0:
                raise ValueError(f"Absolute count must be positive, got: {count}")
    ...
    if hasattr(args, 'encoder_model_path') and args.encoder_model_path is not None:
        kwargs["encoder_model_path"] = args.encoder_model_path
